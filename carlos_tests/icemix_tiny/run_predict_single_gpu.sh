...
# Single-GPU prediction script for RTX6000 nodes
# Run this from within your salloc'd interactive job

echo "=== DeepIce Single-GPU Prediction Script for RTX6000 ==="
...
export OUTPUT_DIR="/storage/home/hcoda1/8/cfilho3/r-itaboada3-0/graphnet/carlos_tests/icemix_tiny/logs/prediction_single_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
# --- Run prediction on single GPU -------------------------------------------
echo "Starting prediction on single GPU (GPU 0)..."
...
python train_icemix_mixed_tiny.py \
       --mode predict \
       --batch-size 512 \
       --num-workers 12 \
       --pin-memory \
