...
checkpoint_callback = ModelCheckpoint(
    dirpath=CHECKPOINT_DIR,                     # Use local checkpoint directory
    filename="best-{epoch:02d}-{val_loss:.4f}",  # File name format
    monitor="val_loss",                         # Monitor validation loss
    mode="min",
    save_top_k=1,                               # Only keep the best model on validation set
    save_last=True                              # Also save the latest checkpoint for resuming training
)
...
def main(
    ...
    archive = os.path.join(CHECKPOINT_DIR, "model_outputs")
    ...
    data_module = GraphNeTDataModulecustom(dataset_reference=SQLiteDataset,
                                    ...
                                    "features": features,
                                    "path":data_paths,
                                    # ["/storage/home/hcoda1/4/jliao74/p-itaboada3-0/jliao74/my_numu_database_part_1 (1).db", "/storage/home/hcoda1/4/jliao74/p-itaboada3-0/jliao74/my_nue_database_part_1 (1).db"],
    ...
    #data_module1 = GraphNeTDataModule(dataset_reference=SQLiteDataset,
     #                                dataset_args = {
      #                              "truth_table": truth_table,
       #                             "pulsemaps": config["pulsemap"],
        #                            "truth": truth,
    ...
    task = JointPositionandDirectionReco(
        ...
        loss_function=JointLoss(alpha=0,position_loss=EuclideanDistanceLoss(),direction_loss=VonMisesFisher3DLoss()),
        )
    model = StandardModel(
        graph_definition=graph_definition,
        backbone=backbone,
    ...
    if os.path.exists(last_ckpt):
        ckpt_path = last_ckpt
        print(f"Found existing checkpoint, resuming from: {ckpt_path}")
    else:
    ...
    # TODO: Uncomment this when we have a way to run predictions on a single GPU
