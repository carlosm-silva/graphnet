    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            if r == 0:
                ...
                merged = sorted({idx for sub in all_indices for idx in sub})
...
def main(
    ...
    task = JointPositionandDirectionReco(
        ...
        loss_function=JointLoss(alpha=0.01,position_loss=EuclideanDistanceLoss(),direction_loss=VonMisesFisher3DLoss()),
        )
    model = StandardModel(
        graph_definition=graph_definition,
        backbone=backbone,
