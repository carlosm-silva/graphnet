...
- IceMix also depends on private `StandardModel`/task contracts: `_tasks`, `_loss_function`, `_get_batch_size`, `shared_step`, optimizer/scheduler fields, and the inherited optimizer step. The focused EMA/LBFGS tests cover most of those wrapper assumptions; the full metrics boundary requires a training/validation batch.
...
- Author supplied 2026-08-13: `docs/graphnet_env/` contains raw PACE exports plus a prefix-free two-stage successor recipe for Python 3.8.20, PyTorch 2.2.0+cu118, PyG 2.6.1, and Lightning 2.4.0. Commit `4394131647b4a581e7d4923361b2814ab9e03ff5` is the raw export's repository provenance, not the handoff revision to check out. Recreation on Phoenix is still unverified.
...
- 2026-08-14: `AUDIT.md` and `AUDIT_PROGRESS.md` are external audit artifacts, not IceMix documentation deliverables, and the documentation maintainer must not edit them.
- 2026-08-14: A handoff instruction to contact the appropriate person is not a blocker. PACE queue/account operations belong to PACE IT or experienced group members; the supplied `docs/graphnet_env/` export covers the inherited software environment; Q004 and senior group members provide practical healthy-run guidance.
- 2026-08-14: Do not spend handoff effort polishing inconsequential wording distinctions such as “verbatim” versus “semantically transcribed” when the technical meaning is already clear.
- 2026-08-14: Most handoff documentation was LLM-generated with OpenAI Codex and ChatGPT through multiple drafting and adversarial/auditor rounds. The author reviewed or skimmed much of it but did not independently verify every claim. The successor must treat it as a fallible evidence map, push back, investigate consequential claims, and apply ordinary scientific skepticism rather than accepting it at face value.
- 2026-08-14: As a research-group member, the successor will have read/write access to the author's full PACE project tree below `/storage/project/r-itaboada3-0/cfilho3`, including the untracked `graphnet/ice_mix/.env`. The successor should inspect that inherited file directly on PACE for private configuration such as `DATA_ROOT`; it must remain out of Git and documentation.
- 2026-08-14: The final audit findings were prioritized as documentation corrections. The missing smoke command is a usability gap rather than a handoff blocker; empty Blocked/Deferred-data ledgers remain correct; minor wording/tier issues should not displace substantive work.

## Open uncertainties

- Exact Phoenix login host, allocation onboarding procedure, queue behavior, and allocation limits are intentionally delegated to current PACE documentation/IT or experienced group members; they are not IceMix documentation blockers.
- Provenance, preprocessing, detailed schemas, and physical units of the production SQLite databases remain data-owner knowledge; the documented contact route to Jiyuan or another senior group member is considered a complete handoff, not a blocked documentation task.
...
- Local runtime sharp edges 2026-08-13: full reduced training reached a 258K-parameter Lightning model but failed before the first batch because `EpochMonitorCallback.on_train_epoch_start` calls `torch.cuda.current_device()` while distributed execution is initialized. The GPU driver/NVML was unavailable, so GPU execution was not tested. `plot_run.py` wrote all/track plots but crashed on the seven-row cascade subset when log-scaling all-NaN/nonpositive binned statistics.
...
- Researcher-owned static findings added 2026-08-14: importing `src/utils.py` mutates the shared mutable `TRUTH.ICECUBE86` list by appending `oneweight`, and `generate_plots.py` recommends the unsafe inherited `run_predict.sbatch` when predictions are absent. No behavior was changed.
...
- Final audit documentation verification 2026-08-14: all tracked IceMix Python files parse; `src/models/layers.py` retains the HEAD executable AST after stripping docstrings; the effective non-comment requirements are unchanged; all checked local Markdown links resolve; and `git diff --check` passes for the documentation-owned changes.
