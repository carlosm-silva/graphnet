...
def run_prediction(
    ...
    """Run complementary-half inference and write artifacts below a run.
        ...
        ``test_fraction`` selects an unseeded random subset of regenerated
        validation events, or regenerated test events when ``use_test_split``
        is true; ``gpus`` is passed to GraphNeT. The model reconstruction also
        hard-codes or defaults several training settings. Two CSV files are
        written below ``run_dir``. The function returns ``None``.
        """
    logger = Logger()
