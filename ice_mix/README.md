...
2. Follow [day-one orientation](docs/tutorials/01_day_one_and_pace.md) before using PACE resources.
3. Read [configuration and scale](docs/configuration.md) before changing a Hydra override.
4. Rebuild the [exported GraphNeT environment](docs/graphnet_env/README.md).
5. Use [the base-training tutorial](docs/tutorials/03_base_training_and_resume.md) for the recommended production workflow.
...
Base training with the standard, token-drop, rotation, or combined data variant is the recommended production path. **VERIFIED-STATIC; author-confirmed 2026-08-13.**
...
The baseline batch size of 256, 12-block/384-channel transformer, 256-pulse cap, DDP setup, and data-loader tuning were selected for an eight-L40S production job. Do not copy the production launch directly to a laptop or small workstation. Use the [single-GPU smoke example](docs/examples/README.md) to check wiring before spending a cluster allocation.
